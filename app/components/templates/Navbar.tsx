...
import add from "@/public/assets/icon-add-task-mobile.svg";
import down_chevron from "@/public/assets/icon-chevron-down.svg";
import up_chevron from "@/public/assets/icon-chevron-up.svg";
  ...
  return (
    <nav className="fixed z-10 flex w-full items-center h-16 md:h-20 xl:h-24 bg-white dark:bg-dark_grey_secondary border-b-2 border-b-lines_light dark:border-b-lines-dark">
      ...
      >
        ...
        >
          <Image src={logo} alt="Mobile Kanban logo" className="mr-4" />
          <h1 className="text-heading_XL text-black dark:text-white pr-2">
            {context?.currentBoard.name}
          </h1>
          {boardListActive ? (
            <Image src={up_chevron} alt="Chevron up" />
          ) : (
            <Image src={down_chevron} alt="Chevron down" />
          )}
        </a>
        {boardListActive ? (
