...
import { Subtasks, Tasks } from '../../types/GlobalTypes';
...
const TaskModal = ({
  ...
  const deleteTask = () => {
    if (context) {
      const currBoard = { ...context.currentBoard };
  ...
  return (
    <Modal
      ...
      onClose={onClose}
    >
      <div className="flex w-full justify-between px-6 md:px-8 overflow-auto">
        <h2 className="text-black text-heading_L dark:text-white self-start">
          {title}
        </h2>
        <a
          className="flex flex-shrink-0 hover:cursor-pointer self-start"
          onClick={() => setDropdown((prev) => !prev)}
        >
          <img
            src={vertical_ellipsis}
