...
import { addNewColumn } from "@/app/utils/eventHandlers";
...
const Board = ({ sidebarActive, toggleSidebar }: BoardProps) => {
  ...
  return (
    <>
      {boardContext ? (
        <div
          className={`${sidebarActive ? "md:pl-64 xl:pl-80" : ""} relative py-20 md:py-24 xl:py-28 flex w-full flex-grow flex-shrink-0 overflow-auto gap-4 bg-background-light dark:bg-background-dark`}
        >
          {!sidebarActive ? (
            <a
            ...
            >
              <Image src={show_eye_icon} alt="Eye to hide sidebar" />
            </a>
          ) : null}
